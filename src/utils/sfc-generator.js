...
const elTemplates = {  //字段组件属性
  ...
  'table-view': (widget, formConfig) => {
    const { buttonType, buttonPlain, buttonRound, buttonCircle, buttonIcon, disabled } = getElAttrs(widget, formConfig)
    return `<table-view ${buttonType} ${buttonPlain} ${buttonRound} ${buttonCircle} ${buttonIcon}
            ${disabled}>${widget.options.label}</table-view>`
  },
...
function genTemplate(formConfig, widgetList, vue3Flag = false) {
  ...
  const isWrapAttr = !!formConfig.isWrap ? `:inline="true"` : ''
  let childrenList = []
...
export const genSFC = function (formConfig, widgetList, beautifier, vue3Flag = true) {
  ...
  return `<!-- 
基于开源项目VForm3二次开发,详情请访问：https://www.vform666.com
aaron 666666
-->
